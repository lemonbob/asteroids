...
<!--<i-if i-prop:condition='{this.data.isGameEnded}'>
	<div class="test" test1="{this.data.score}">
		<button i-event='{"click":"this.test2"}'>SLOT BUTTON1 {this.data.score}</button>
		<my-button i-props='{"score":"this.data.score"}'></my-button>
	</div>
</i-if>-->
<i-for2 i-prop:of='{this.data.testArray}' value="item" key="key">
	<div class="test">
		<my-button i-props='{"score":"this.data.score"}'></my-button>
	</div>
</i-for2>
...
class IAPP extends IMPRESS {
	...
	gameLoop(timeStamp) {
		...
		//this.iSetState('this.data.hiScore', Math.round(1000 / (timeStamp - this.data.previousTimeStamp ?? 0)));
